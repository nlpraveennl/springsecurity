	...
	public List<IssueVO> listIssue()
	{
		try (Session session = sessionFactory.openSession();)
		{
			TypedQuery<IssueVO> query = session.createQuery(""
					+ "SELECT NEW com.gmail.nlpraveennl.vo.IssueVO(i.id, i.name, i.summary, i.description, i.issueType, i.reporter, i.assignee, i.projectId, udrep.userName, udasn.userName, proj.name)"
					+ "FROM Issue i "
